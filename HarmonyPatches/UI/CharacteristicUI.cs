...
namespace ReBeat.HarmonyPatches.UI {
	...
	class CharacteristicUI {
		...
		static void Prefix(ref IEnumerable<BeatmapCharacteristicSO> beatmapCharacteristics) {
			if (!IsCustomLevel) return;
			Plugin.Log.Info("Setting data");
			beatmapCharacteristics = beatmapCharacteristics.Where(x => (Config.Instance.Enabled && x.serializedName.StartsWith("ReBeat_")) || (!Config.Instance.Enabled && !x.serializedName.StartsWith("ReBeat_"))).ToList();
		}
