		...
		static void RegisterAll(BeatmapCharacteristicCollectionSO collection) {
			...
			foreach (var characteristic in allCharacteristics) {
				...
				SongCore.Collections.RegisterCustomCharacteristic(combined, $"ReBeat! {characteristic.name}",
					// TODO: temporary fix until I can figure out a way to have localization work
					$"ReBeat! {characteristic.serializedName}" /*$"ReBeat! {Localization.Get(characteristic.descriptionLocalizationKey)}"*/,
					$"ReBeat_{characteristic.serializedName}",
